...
def tguri(index, board):
    ...
    if downloadurl : 
        ...
        name = str(index)
        f = open('/volume1/downloads/'+name+'.torrent', 'wb')
        f.write(data)
        f.close()
...
def tosarang(ip,index, board):
    ...
    if url2 : 
        ...
        insertSeq(ip,'tosarang', board, index, "한국영화", url2)
    else:
        print('end'+name+'no data')        
...
while 1 :
    ...
    try: 
        seq = getSeq(ip,'tosarang', 'torrent_movie_kor')
        tosarang(ip,seq, 'torrent_movie_kor')
    except:
        print('tosarang except torrent_movie_kor')
    
        
    try: 
        seq = getSeq(ip,'tosarang', 'torrent_kortv_ent')
        tosarang(ip,seq, 'torrent_kortv_ent')

    except:
        print('tosarang except torrent_kortv_ent')
